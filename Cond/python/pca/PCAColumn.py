...
class PCAColumn(object):
    ...
    # For prediction
    def load_all_vector(self, data, tags, all_deleted_list_dict):
        ...
        ori_tags = data.columns.tolist()

        for tag in tags:
            if tag not in ori_tags:
                continue
            if not all_deleted_list_dict.has_key(tag):
                continue
    ...
    def process_pca_err_columns(self, X, tag):
        if tag == 'varpre1st' or tag == 'varpre1st' or tag == 'varpre2nd'\
               or tag == 'tppre1st' or tag == 'tppre2nd' or tag == 'tppre3rd':  #or tag == 'nstmt0': only for math_59's training error

            X = (X - np.mean(X)) / np.std(X)

        # if tag == 'methodname' and self.__configure__.prognm_and_id == 'math_106':
        #     X = (X - np.mean(X)) / np.std(X)
    ...
    def train_model(self, all_model_set, tag, data, labels=None):
        ...
        logging.info('{} DI REDUCTION, FROM {} TO {}'.format(tag, data.shape,  new_data.shape))

        df = self.transform_to_pca_df(tag, new_data)

        all_model_set[tag] = model
        del data
        gc.collect()

        return df

    def load_model(self, all_model_set, tag, data):
        pca_model = all_model_set[tag]
    ...
    @staticmethod
    def get_pca_dataframe(ori_file_data, config, mission_type, tags, all_model_set, is_training=False, all_deleted_list_dict=None):
        """
        ...
        if is_training:
            ...
            all_bag_encoders = pca_col.load_all_vector(ori_file_data, tags, all_deleted_list_dict)

        ori_tags = ori_file_data.columns.tolist()

        pca_df = None
        for tag in tags:
            if tag not in ori_tags:
                continue
            if not all_bag_encoders.has_key(tag):
                continue

            if is_training:
                # pca analysis
                curr_df = pca_col.train_model(all_model_set,
                                              tag,
                                              all_bag_encoders[tag],
                                              y)
                del all_bag_encoders[tag]
            else:
                curr_df = pca_col.load_model(all_model_set,
                                             tag,
                                             all_bag_encoders[tag])
